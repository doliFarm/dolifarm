...
/**
 * \file        htdocs/modulebuilder/template/class/farm.class.php
 * \ingroup     dolifarm
 * \brief       This file is a CRUD class file for Farm (Create/Read/Update/Delete)
 */
 ...
 * Class for Farm
 */
class Farm extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'dolifarm_farm';
	...
	/**
	 * @var string String with name of icon for farm. Must be the part after the 'object_' into object_farm.png
	 */
	public $picto = 'farm@dolifarm';
	...
	public $fields = array(
		...
		'fk_soc' 		=> array('type'=>'integer:Societe:societe/class/societe.class.php:1:( (status:=:1) AND (entity:LIKE:__SHARED_ENTITIES__) )', 'picto'=>'company', 'label'=>'ThirdParty', 'visible'=> 1, 'enabled'=>1, 'position'=>50, 'notnull'=>-1, 'index'=>1, 'help'=>'LinkToThirparty', 'validate'=>1, 'css'=>'maxwidth500 widthcentpercentminusxx'),
	...
	//  */
	// public $table_element_line = 'dolifarm_farmline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_farm';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Farmline';
	...
	//  */
	// protected $childtablesoncascade = array('dolifarm_farmdet');

	// /**
	//  * @var FarmLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->dolifarm->farm->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolifarm->farm->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolifarm->farm->farm_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'farm/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'farm/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->dolifarm->dir_output.'/farm/'.$oldref;
				$dirdest = $conf->dolifarm->dir_output.'/farm/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->dolifarm->dir_output.'/farm/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Farm").'</u>';
		...
		$url = dol_buildpath('/dolifarm/farm_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('farmdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new FarmLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_farm = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->MYMODULE_MYOBJECT_ADDON)) {
			$conf->global->MYMODULE_MYOBJECT_ADDON = 'mod_farm_standard';
		}
	...
	/** Return the TE_FARM code defined in the dictionary
	*/
	
	public function typentFarm() {		
...
/**
 * Class FarmLine. You can also remove this and generate a CRUD class for lines objects.
 */
class FarmLine extends CommonObjectLine
{
	// To complete with content of an object FarmLine
