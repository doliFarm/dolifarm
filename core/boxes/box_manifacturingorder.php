 ...
 */
class box_manifacturingorder extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "manifacturingorderbox";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("ManifacturingOrderWidget");
	...
	public function loadBox($max = 5)
	{
		...
		require_once DOL_DOCUMENT_ROOT.'/custom/dolilogistic/class/productionorders.class.php';
		require_once DOL_DOCUMENT_ROOT.'/commande/class/commande.class.php';

		//require_once DOL_DOCUMENT_ROOT.'/core/lib/functions.lib.php';
		require_once DOL_DOCUMENT_ROOT.'/core/lib/date.lib.php';

		$productionorders = new Productionorders($db);
		// Use configuration value for max lines count
		$this->max = $max;
					// Populate the head at runtime
			$text = $langs->trans("ManifacturingOrderBoxHeader",$max);
			$this->info_box_head = array(
			 ...
			 $this->info_box_contents[0] =  array( // First line
																...
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('Order')
																),
																2 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('DateStart')
																),
																3 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('DateEnd')
																),
																4 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
		...
		$objlist = fetchobj('dolifarm_productionorders', 'date_start', $max);
		//print_r($objlist);
		if ($objlist) {
			$order = new Commande($db);
			for ($i=0;$i<count($objlist);$i++) {
			// Populate the contents at runtime
						$order->fetch($objlist[$i]['fk_order']);
						$this->info_box_contents[$i+1] =  array( // First line
																0 => array( // First Column
																	...
																	'text' => $objlist[$i]['ref'],
																	// Link on 'text' and 'logo' elements
																	'url' => DOL_URL_ROOT.'/custom/dolitrace/productionorders_card.php?id='.$objlist[$i]['rowid'],
																	// Link's target HTML property
																	'target' => '',
																	// Fist line logo (deprecated. Include instead logo html code into text or text2, and set asis property to true to avoid HTML cleaning)
																	//'logo' => 'monmodule@monmodule',
																...
																1 => array( // TR
																'tr' => 'class="left"',
																'text' => $order->getNomUrl()
																),
																2 => array( // TR
																'tr' => 'class="left"',
																'text' => dol_print_date(dol_stringtotime($objlist[$i]['date_start']))
																),
																3 => array( // TR
																'tr' => 'class="left"',
																'text' => dol_print_date(dol_stringtotime($objlist[$i]['date_end']))
																),
																4 => array( // TR
																'tr' => 'class="left"',
