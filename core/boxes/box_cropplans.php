 ...
 */
class box_cropplans extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "cropplansbox";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("CropPlansWidget");
	...
	public function loadBox($max = 5)
	{
		...
		$cropplans = new Cropplans($db);
		// Use configuration value for max lines count
		$this->max = $max;

		// Populate the head at runtime
		$text = $langs->trans("CropPlansBoxHeader",$max);
		...
		$this->info_box_contents[0] =  array( // First line
																...
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('StartDate')
																),
																2 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('FinishDate')
																),
																3 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('EstimatedYield')
																),
																4 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
																'text' => $langs->trans('YieldToDate')
																),
																5 => array( // TR
																'tr' => 'class="left"',
																'td' => 'class=bold',
		...
		$objlist = fetchobj('dolifarm_cropplans', 'finishdate', $max);
		if ($objlist) {	
			...
			// Populate the contents at runtime
						$this->info_box_contents[$i+1] =  array( // First line
																0 => array( // First Column
																	...
																	'text' => $objlist[$i]['ref'],
																	// Link on 'text' and 'logo' elements
																	'url' => DOL_URL_ROOT.'/custom/dolitrace/cropplans	_card.php?id='.$objlist[$i]['rowid'],
																	// Link's target HTML property
																	'target' => '',
																	// Fist line logo (deprecated. Include instead logo html code into text or text2, and set asis property to true to avoid HTML cleaning)
																	//'logo' => 'monmodule@monmodule',
																...
																1 => array( // TR
																'tr' => 'class="left"',
																'text' => dol_print_date(dol_stringtotime($objlist[$i]['startdate']))
																),
																2 => array( // TR
																'tr' => 'class="left"',
																'text' => dol_print_date(dol_stringtotime($objlist[$i]['finishdate']))
																),
																3 => array( // TR
																'tr' => 'class="left"',
																'text' => $objlist[$i]['estimatedyield']
																),
																4 => array( // TR
																'tr' => 'class="left"',
																'text' => $objlist[$i]['yieldtodate']
																),
																5 => array( // TR
																'tr' => 'class="left"',
