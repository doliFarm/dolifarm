 ...
 */
class box_certificates extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "certificates";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("BoxCertificatesexpiringWidget");
	...
	public function loadBox($max = 5)
	{
		...
		$text = $langs->trans("BoxCertificatesHeader",$max);
		...
		$this->info_box_contents[0] =  array( // First line
															...
															'tr' => 'class="left"',
															'td' => 'class=bold',
															'text' => $langs->trans('BodyCA')
															),
															2 => array( // TR
															'tr' => 'class="left"',
															'td' => 'class=bold',
															'text' => $langs->trans('ExpirationDate')
															)
												);
		$farmCertificateExpiring = listExpiringCertificate();
		$soc = new Societe($db);
		if ($farmCertificateExpiring) {	
			for ($i=0;$i<count($farmCertificateExpiring) && $i < $max;$i++) {
						$result = $soc->fetch($farmCertificateExpiring[$i]['farmid'],$ref);
			// Populate the contents at runtime
						$this->info_box_contents[$i+1] =  array( // First line
																0 => array( // First Column
																	...
																	'text' => $soc->nom,
																	// Link on 'text' and 'logo' elements
																	'url' => DOL_URL_ROOT.'/societe/card.php?socid='.$farmCertificateExpiring[$i]['farmid'],
																	// Link's target HTML property
																	'target' => '',
																	// Fist line logo (deprecated. Include instead logo html code into text or text2, and set asis property to true to avoid HTML cleaning)
																	//'logo' => 'monmodule@monmodule',
																...
																1 => array( // TR
																'tr' => 'class="left"',
																'text' => bodyCA($farmCertificateExpiring[$i]['bodyCA'],$soc)
																),
																2 => array( // TR
																'tr' => 'class="left"',
																'text' => dol_print_date($farmCertificateExpiring[$i]['expirationdate'])
																)
																);
			}
		}else {
