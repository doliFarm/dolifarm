...
llxHeader("", $langs->trans("DoliFarmArea"));

print load_fiche_titre($langs->trans("DoliFarmArea"), '', 'dolifarm.png@dolifarm');
...
// Draft MyObject
if (! empty($conf->dolifarm->enabled) && $user->rights->dolifarm->read)
{
	$langs->load("orders");
...
if (! empty($conf->dolifarm->enabled) && $user->rights->dolifarm->read)
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."dolifarm_myobject as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
